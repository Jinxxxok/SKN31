...
select distinct job, dept_name from emp; -- 여러 컬럼 디스팅트는 행기준 중복처리


-- EMP 테이블에서 emp_id는 직원ID, emp_name은 직원이름, hire_date는 입사일, salary는 급여, dept_name은 소속부서 별칭으로 조회결과를 출력 한다.
select emp_id as 직원ID,  -- as는 생략가능하다.
       emp_name as 직원이름, -- 별칭에 공백이 들어가면 " "로 묶어준다 ( 쌍따옴표 ) 
       hire_date as 입사일, 
       salary as 급여, 
       dept_name as 소속부서
from   emp;
...
where hire_date between '2004-01-01' and '2004-12-31'; -- 년-월-일 2004-01-01 ~ 2004-12-31 
...
-- EMP 테이블에서 업무(job)에 'MAN'이 들어가는 직원들 중에서 부서(dept_name)가 'Shipping' 이거나 2005년이후 입사한  직원들을 조회.
select *
from emp
