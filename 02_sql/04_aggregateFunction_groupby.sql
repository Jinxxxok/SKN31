...
select count(dept_name) from emp; -- null 제외한 값의 갯수를 조회함.
...
select distinct ifnull(dept_name, 'a') from emp;
...
-- select			  5. 무슨 무슨 컬럼을 조회할것인지
-- from               1. 테이블을 찾는다.
-- where			  2. 전체 데이터에서 조건에 맞는(원하는) 행들을 찾는것
-- group by			  3. 그룹으로 나눠줌
-- having			  4. 어떤 그룹을 체크할 것인지에 대한 그룹을 걸러주기(조건들)
-- order by 		  6. 마지막 정렬을 어떻게 할 것 인지
