...
CREATE TABLE emp(
    ...
    CONSTRAINT fk_emp_dept FOREIGN KEY(dept_id) REFERENCES dept(dept_id) ON DELETE SET NULL, -- ON DELETE CASCADE 부모의 pk가 지워지면 모든 자식테이블의 fk데이터들을 다 지워버린다.
    CONSTRAINT fk_emp_job FOREIGN KEY(job_id) REFERENCES job(job_id) ON DELETE SET NULL, -- ON DELETE SET NULL 부모의 pk 한 행이 지워지면 자식테이블의 fk데이터들을 Null로 채운다.
    CONSTRAINT fk_emp_mgr FOREIGN KEY(mgr_id) REFERENCES emp(emp_id) ON DELETE SET NULL
); -- on delete cascade 
-- constraint 제약조건_이름 제약조건을_적용할(컬럼명) 
