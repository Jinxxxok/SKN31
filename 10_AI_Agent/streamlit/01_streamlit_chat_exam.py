...
if "chat_history" not in st.session_state: # Key 조회 (딕셔너리일 경우엔 키가 있냐 없냐 조회를 함)
    st.session_state['chat_history'] = [] # 없으면 chat_history라는 이름으로 빈 list 생성. / list[dict]  << 형태로 만들어주려고 함.
    # 개별 메세지를 dictionary로 저장. {"role":"user/ai", "content":"메세지"} - OpenAI 메세지 형식
