...
def vote(request):
    ...
    if choice_id != None: # 선택된 보기가 넘어온 경우.
        ...
        ## Web Browser에게 결과를 보여주는 View로 이동하도록 요청. 그래서 새로고침을 하더라도 투표가 다시 되는 것을 막도록 한다.

        url = "/polls/vote_result/"+str(question_id) # redirect할 URL
        # urls.py의 설정된 URL을 조회 -> reverse(url mapping 설정이름)
        # url mappiong 설정이름: app_name: 설정 name
        url = reverse("polls:vote_result", args = [question_id]) # path parameter: args에 순서대로 입력
        print(">>>>>>> reverse url:", url)
        return redirect(url) # 응답상태 코드가 302인 HttpResponse를 반환.
...
def vote_create_old(request):
    ...
    if http_method == "GET":
        ...
        ## 조회 메소드: get(이름) - 조회결과가 한개인 경우, return: str 
        ...
        # 요청파라미터 검증
        ## question_text로 넘어온 값이 없거나 (있는데 빈 문자열이라면)
        ...
        ## 보기 요청 파라미터 검증 - 보기가 없거나 두개 미만이라면
        ...
        ## transaction 처리 -> 질문과 보기가 모두 insert되거나 실패하면 모두 insert안되도록 보장    
